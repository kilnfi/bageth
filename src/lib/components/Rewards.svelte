<script lang="ts">
  ...
  import Chart from "./RewardsChart.svelte";
...
<div class="w-11/12 xl:w-8/12 my-6 flex flex-col gap-y-10">
  ...
  <Table loading={$navigating?.type === "goto"}>
    <thead slot="head">
      ...
      <th
        ><span use:tooltip={{ content: help.balance }}>Staked balance</span></th
      >
    ...
    <tbody slot="body">
      {#each data?.data ?? [] as item, idx}
        <tr class="group/row relative" style="animation-delay: {idx * 1}ms;">
          ...
          <td>{item.cl_apy?.toFixed(2)}%</td>
          <td>{item.el_apy?.toFixed(2)}%</td>
